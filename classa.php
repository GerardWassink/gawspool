...
print "<h1>" . $sysnam . "/370 class A output</h1>";	/* --- get system name			--- */

print "<table border='0'>";				/* --- Start of table			--- */
print "<tr bgcolor='#1a5ab3'><th>&nbsp;#&nbsp;&nbsp;</th><th>jobnum</th><th>jobname</th><th>view</th><th>purge</th></tr>";
...
foreach($files as $filename) {				/* --- cycle thru file names		--- */
  ...
  if ( is_writable($fqn) ) {				/* --- check writability		--- */
    $pur_txt = "<a href='./purge_job.php?fn=" . $fqn . "&jn=" . $jobnum . "'>purge</a>";
  } else {
    $pur_txt = "<a href='./purge_job.php?fn=" . $fqn . "&jn=" . $jobnum . "'>noauth</a>";
  }
  ...
  print "<td align='center'><a href='" . $fqn . "' target='_blank'>view</a></td>"; /* --- view?	--- */
...
print "<h3>Usage:</h3>";
print "<ul>";
print "<li>Click on desired action near the name to view the job in a seperate window (close window to return here)</li>";
print "</ul>";
